...
void dammy_genGraph(int N, int G[N][N]);
...
void rk4(int n, int N, double dt, double x[N], double y[N], double u[N], double v[N], int fixed_num, int fixed_p[fixed_num], int G[N][N], int p2l_mat[N][N],double force_x[N], double m[N], int M, double k[M], double gamma1[M], double natu_l);
...
double updateLyapunovExponent(int time_steps, double *array1, double *array2, int M, double initial_d, double *sum_log);
...
int main(int argc, char *argv[]){
  ...
  const int N = atoi(argv[2]); // number of mass points
...
// washout, learning, evaluating term (time steps)
  ...
  //  updateNarma(input, o_nrm2, o_nrm10, o_nrm20);
    rk4(n, N, dt, x, y, u, v, fixed_num, fixed_p, G, p2l_mat, force_x, m, M, k, gamma1, natu_l);
    rk4(n, N, dt, x_d, y_d, u_d, v_d, fixed_num, fixed_p, G, p2l_mat, force_x, m, M, k, gamma1, natu_l);
    // printf("%d %f %f %f %f\n",n,x[8],y[8],x_d[8],y_d[8] );
    // printf("%d %f %f\n",n,input[0], force_x[N-1] );
    getSpringLength(l,x,y,N,G);
    getSpringLength(l_d,x_d,y_d,N,G);
    // printf("%.12lf\n",l_d[0]);
    lyapunov = updateLyapunovExponent(n,l,l_d, M, initial_d, &sum_log);
    // printf("%14.12lf %14.12lf\n",sum_log,lyapunov);
...
//printf("\n");
  for(idx2=0;idx2<N;idx2++){
    ...
    //  printf("arrayl_idx=%d, G[%d][%d]=%d, p2l_mat[%d][%d]=%d\n", arrayl_idx, idx1, idx2, G[idx1][idx2],idx1, idx2, p2l_mat[idx1][idx2]);
