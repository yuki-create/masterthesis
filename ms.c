...
void updateLearnigData(int sample, int time_steps, int M, int WASHOUT, int LEARNING, double l[M+1],double o_nrm2[3], double o_nrm10[11], double o_nrm20[21],double o_nrm30[31], double *T, double *L){
  int i=0;
  int idx = time_steps/sample;
  T[idx-WASHOUT] = o_nrm2[0];
  T[idx-WASHOUT+LEARNING] = o_nrm10[0];
  T[idx-WASHOUT+LEARNING*2] = o_nrm20[0];
  T[idx-WASHOUT+LEARNING*3] = o_nrm30[0];
  //printf("%d  %d  %d  %d\n",idx-WASHOUT, idx-WASHOUT+LEARNING, idx-WASHOUT+LEARNING*2, idx-WASHOUT+LEARNING*3);
   // printf("%lf %lf %lf\n",T[idx-WASHOUT], T[idx-WASHOUT+LEARNING], T[idx-WASHOUT+LEARNING*2]);
  for(i=0;i<(M+1);i++){
    L[idx-WASHOUT+LEARNING*i] = l[i];
  //    printf("%lf ",L[idx-WASHOUT+LEARNING*i]);
  }
//  printf("%d\n",idx-WASHOUT+LEARNING*(i-1));
  // printf("\n");
}
